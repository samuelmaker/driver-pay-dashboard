...
async function getPlansForMonth(year, month) {
  ...
  if (plansCache.data[cacheKey] && plansCache.timestamps[cacheKey]) {
    ...
    if (age < CACHE_DURATION_MS) {
      console.log(`Using cached plans for pay period ${cacheKey}`);
  ...
  console.log(
    `Fetching plans for pay period ${cacheKey} (${startDate.day}/${startDate.month}/${startDate.year} - ${endDate.day}/${endDate.month}/${endDate.year}) from Spoke API`
  );
  ...
  // Cache the results
  plansCache.data[cacheKey] = periodPlans;
